#!/usr/bin/env python3
"""
TFT Hourly Training — Beach Occupancy Prediction

Data is filtered 8:00-20:00 → 12 hours/day.
Trains per-horizon models: tft_model_3d/, tft_model_10d/, tft_model_15d/
Each horizon gets its own feature selection and hyperparameter optimization.
Each model runs in a fresh subprocess (no CUDA memory corruption).
...
Output structure:
    <prefix>_<timestamp>/
        ...
        tft_model_3d/     — model artifacts
        tft_model_10d/
        tft_model_15d/

Usage:
    python tft_train.py
    python tft_train.py --phase0              # include horizon limit analysis
    python tft_train.py --data-dir DIR        # custom data directory
    python tft_train.py --prefix my_run       # custom output prefix
    python tft_train.py --cuda-device 0       # force specific GPU
    python tft_train.py --deploy-only         # skip optimization, use known config
    python tft_train.py --eval-dirs clean_datasets clean_dataset_backup  # evaluate on these
    python tft_train.py --skip-eval           # no evaluation
    python tft_train.py --worker <dir>        # internal subprocess
"""
...
def make_run_dir(prefix):
    """Create a unique run directory: <prefix>_<YYYYMMDD_HHMMSS>/"""
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    run_dir = f'{prefix}_{timestamp}'
...
def _worker_main(work_dir):
    ...
    tft = TFT(
        ...
        hidden_size=kw.get('hidden_size', 64), n_head=kw.get('n_head', 4),
    ...
    if mode == 'cv':
        ...
        cv = cv.reset_index().dropna(subset=['y', 'TFT'])
        cv['TFT'] = cv['TFT'].clip(lower=0)
        ...
        save_path = args.get('save_path', 'tft_model/nf_model')
...
def safe_fit(panel_df, static_df, futr, hist, input_size, horizon,
             save_path='tft_model/nf_model', cuda_device='', timeout=None, **kwargs):
    work_dir = tempfile.mkdtemp(prefix='tft_w_')
    try:
        with open(f'{work_dir}/input.pkl', 'wb') as f:
            pickle.dump({
...
def phase4(label, horizon, cache_df, django_df, all_futr, om_hist,
    ...
    model_dir = f'{run_dir}/tft_model_{label}'
...
def parse_args():
    parser = argparse.ArgumentParser(description='TFT Hourly Training')
    parser.add_argument('--data-dir', default='clean_datasets', help='Data directory')
    parser.add_argument('--prefix', default='tft_run', help='Output directory prefix (default: tft_run)')
...
KNOWN_DATASETS = {
    'clean_datasets': 'default',
    'clean_datasets_new': 'new',
    'clean_dataset_backup': 'old',
}
...
def resolve_eval_dirs(args):
    ...
    if args.eval_dirs is not None:
        ...
        for d in args.eval_dirs:
            if os.path.isdir(d):
                dirs[d] = KNOWN_DATASETS.get(d, os.path.basename(d))
            else:
                log(f"  Eval dir not found, skipping: {d}", 'WARN')
        return dirs
    # Default: train dir + all known dirs that exist
    dirs = {}
...
def main(args):
    ...
    log("TFT Training — Per-Horizon Optimization")
...
def main_deploy_only(args):
    ...
    log("TFT Training — Deploy Only (skip optimization)")
    log(f"Target models: {TARGET_HORIZONS}")
    log(f"Run dir:  {run_dir}")
    log(f"CUDA device: {cuda_device if cuda_device else 'CPU'}")
